...
public class KeywordLibrary{
	 ...
	 * Return Type: WebElement
	 * Description: This method returns the Web Element after forming the By selector using the identifier type and element id
	 */
	 ...
	 * Method Name: getWebElement
	 * Return Type: List WebElement
	 * Description: This method returns the list of Web Elements after forming the By selector using the identifier type and element id
	 */
	...
	//Creating JavaScriptExecuter Interface
      ...
      // Starting loop to highlight and then de-highlight the web element
	   for (int iCnt = 0; iCnt < 2; iCnt++) {
		   //Execute Javascript
		   if(iCnt == 0){
			   js.executeScript("arguments[0].setAttribute('style', arguments[1]);", findElementByType(driver, scenario, testCase, homePath, currentIteration.intValue(), objectId, browser, passScreenshot, browserFolder), "color: black; border: 10px solid black;");
			   Report reportObject = new Report();
	...
	public void visualVerification(String homePath, String testCase, String scenario, String browser, String objectId, String objectName, String dataValue, String onPassLog, String onFailLog, WebDriver driver, String passScreenshot, Integer currentIteration, Boolean error, String browserFolder)
	{
		Report reportObject = new Report();
		Util utilObject = new Util();
		
		if((utilObject.getValueFromConfigProperties("VisualVerification_Applitool").toUpperCase().equals("YES")) && 
			(utilObject.getValueFromConfigProperties("VisualVerification_Galen").toUpperCase().equals("YES")))
        {
			throw new RuntimeException("Switch for multiple visual automation i.e. Galen / Applitool cannot be YES simultaneously");
        }  
		
		

		if(utilObject.getValueFromConfigProperties("VisualVerification_Galen").
				toUpperCase().equals("YES"))
		{
			String ReportsFolder=browserFolder+dataValue;
			String LocSpecFile=System.getProperty("user.dir")+ "\\src\\test\\resources\\Spec\\" + dataValue + ".gspec";
			List<String> devicetype= new ArrayList<String>();
			
			
			String Reportlink="<a href=\"" + ReportsFolder + "/report.html\">ReportLink</a>";
			
			try {
	
				if(utilObject.getValueFromConfigProperties("RunMobileAutomation").toUpperCase().equals("YES"))
	            {           devicetype.add("mobile");
	                  
	            }
	            if(utilObject.getValueFromConfigProperties("RunWebAutomation").toUpperCase().equals("YES"))
	            {           
	                  devicetype.add("desktop");
	            }
	
				
			LayoutReport layoutreport = Galen.checkLayout(driver,LocSpecFile,devicetype);
			List<GalenTestInfo> tests = new ArrayList<GalenTestInfo>();
			GalenTestInfo test= GalenTestInfo.fromString(objectName);
			test.getReport().layout(layoutreport, objectName);
			tests.add(test);
			new HtmlReportBuilder().build(tests, ReportsFolder);
			
			if(layoutreport.errors()>0){
				reportObject.Log("Visual Verification for"+objectName, Reportlink, Status.FAIL, driver, testCase, scenario, browser, passScreenshot, browserFolder);
			}
			else{
			
				reportObject.Log("Visual Verification for"+objectName, Reportlink, Status.PASS, driver, testCase, scenario, browser, passScreenshot, browserFolder);
			}
		...
		{
	            Eyes eyes = new Eyes();
	            eyes.setApiKey(utilObject.getValueFromConfigProperties("ApiKey"));

	            try {
	            	
	            	int X_Coordinate=Integer.parseInt(utilObject.getValueFromConfigProperties("X_Coordinate"));
	            	int Y_Coordinate=Integer.parseInt(utilObject.getValueFromConfigProperties("Y_Coordinate"));
	                
	            	driver = eyes.open(driver, objectName,dataValue, new RectangleSize(X_Coordinate, Y_Coordinate));

	                eyes.checkWindow(dataValue);
 
	               eyes.close();
	       		reportObject.Log("Visual Verification for"+objectName, dataValue, Status.PASS, driver, testCase, scenario, browser, passScreenshot, browserFolder);
	            }
	           catch(Exception e) {
	                // Abort test in case of an unexpected error.
	                eyes.abortIfNotClosed();
		       		reportObject.Log("Visual Verification for"+objectName, dataValue, Status.FAIL, driver, testCase, scenario, browser, passScreenshot, browserFolder);
	            //    driver.close();
	            }
	...
	public void enter_Custom_CalendarDate(String homePath, String testCase, String scenario, String browser, String objectId, String objectName, String dataValue, String onPassLog, String onFailLog, WebDriver driver, String passScreenshot, Integer currentIteration, Boolean error, String browserFolder){
		Report reportObject = new Report();
		Util utilObject = new Util();
		String mm = utilObject.getValueFromConfigProperties("appDate").split("-")[0].trim();
		String dd = utilObject.getValueFromConfigProperties("appDate").split("-")[1].trim();
		String yyyy = utilObject.getValueFromConfigProperties("appDate").split("-")[2].trim();
